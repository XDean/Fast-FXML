...
public class FXMLLoader {
  ...
  private abstract class Element {
    ...
    /**
     * Creates an array of given type and populates it with values from a string where tokens are
     * separated by ARRAY_COMPONENT_DELIMITER. If token is prefixed with RELATIVE_PATH_PREFIX a
     * value added to the array becomes relative to document location.
     */
     ...
     * ARRAY_COMPONENT_DELIMITER. If token is prefixed with RELATIVE_PATH_PREFIX a value added to
     * the list becomes relative to document location.
     */
    ...
    private void processPropertyHandler(String attributeName, String handlerValue) throws LoadException {
      ...
      if (i != j) {
        ...
        if (handlerValue.startsWith(CONTROLLER_METHOD_PREFIX)) {
          final MethodHandler handler = getControllerMethodHandle(handlerValue, SupportedType.PROPERTY_CHANGE_LISTENER,
              SupportedType.EVENT);
          if (handler != null) {
            if (handler.type == SupportedType.EVENT) {
              ...
              propertyModel.addListener((ChangeListener<Object>) (observable, oldValue, newValue) -> handler.invoke(new Event(value, null, Event.ANY)));
            } else {
  ...
  private class ScriptElement extends Element {
    ...
    public void processAttribute(String prefix, String localName, String value)
      ...
      if (prefix == null
        ...
        throw constructLoadException(prefix == null ? localName
            : prefix + ":" + localName
                + " is not a valid attribute.");
      }
    }
  ...
  /**
   * Escape prefix for escaping special characters inside attribute values. Serves as an escape for
   * {@link #ESCAPE_PREFIX}, {@link #RELATIVE_PATH_PREFIX}, {@link #RESOURCE_KEY_PREFIX,
   * {@link #EXPRESSION_PREFIX}, {@link #BI_DIRECTIONAL_BINDING_PREFIX}
   *
   * @since JavaFX 2.1
   */
  ...
  /**
   * Sets the root of the object hierarchy. The value passed to this method is used as the value of
   * the <tt>&lt;fx:root&gt;</tt> tag. This method must be called prior to loading the document when
   * using <tt>&lt;fx:root&gt;</tt>.
   *
  ...
  /**
   * Sets the controller associated with the root object. The value passed to this method is used as
   * the value of the <tt>fx:controller</tt> attribute. This method must be called prior to loading
   * the document when using controller event handlers when an <tt>fx:controller</tt> attribute is
  ...
  /**
   * Loads an object hierarchy from a FXML document. The location from which the document will be
   * loaded must have been set by a prior call to {@link #setLocation(URL)}.
   * <p>
   * When the "template" flag is set to <tt>false</tt> (the default), this method will clear the
   * imports before loading the document's content. When "template" is <tt>true</tt>, the imports
   * will not be cleared, and the root value will be set to <tt>null</tt> before the content is
   * loaded. This helps improve performance on subsequent loads by eliminating the overhead of
   * loading the classes referred to by the document.
   *
    ...
    try {
      ...
      if (controller != null) {
        if (controller instanceof Initializable) {
          ...
          Map<String, Field> controllerFields = controllerAccessor.getControllerFields();
  ...
  public ParseTraceElement[] impl_getParseTrace() {
    ...
    int i = 0;
    for (FXMLLoader loader : loaders) {
      parseTrace[i++] = new ParseTraceElement(loader.location, (loader.current != null) ? loader.impl_getLineNumber() : -1);
    }
  ...
  private static <T> T loadImpl(URL location, ResourceBundle resources,
    ...
    FXMLLoader fxmlLoader = new FXMLLoader(location, resources, builderFactory,
   ...
   * @return number &lt; 0 if runtime version is lower, 0 when both versions are the same, number
   *         &gt; 0 if runtime is higher version
   */
  ...
  private static final class ControllerAccessor {
    ...
    void setCallerClass(final Class<?> callerClass) {
      final ClassLoader newCallerClassLoader = (callerClass != null) ? callerClass.getClassLoader()
          : null;
    ...
    private static int getAccess(final int fullModifiers) {
      final int untransformedAccess = fullModifiers & (Modifier.PRIVATE | Modifier.PROTECTED
          | Modifier.PUBLIC);
